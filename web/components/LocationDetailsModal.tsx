...
function MediaCell({ type }: { type: "PHOTO" | "VIDEO" | "KEEP" }) {
  const color = type === "PHOTO" ? "#0ea5e9" : type === "VIDEO" ? "#8b5cf6" : "#16a34a";
  return (
    <div style={{
      aspectRatio: "4/3", borderRadius: 6,
      background: "rgba(160,110,30,0.04)",
      border: "1px solid rgba(160,110,30,0.12)",
      display: "flex", flexDirection: "column",
      alignItems: "center", justifyContent: "center", gap: 4,
    }}>
      <span style={{ fontFamily: "monospace", fontSize: 8, color: `${color}66`, letterSpacing: "0.12em" }}>
        {type}
      </span>
    </div>
  );
      ...
      >
        ...
        <div style={{ padding: "20px 20px 16px", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 12, flexWrap: "wrap" }}>
            <CountBadge icon={<IconPhoto />} count={loc.media.photos} label="photos" />
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "16px 20px" }}>
          ...
          {/* Media grid */}
          {loc.media.photos === 0 && loc.media.videos === 0 ? (
            <div style={{ fontFamily: "monospace", fontSize: 10, color: DIM, textAlign: "center", padding: "24px 0" }}>
              No media added yet
            </div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, marginBottom: 16 }}>
              {Array.from({ length: Math.min(loc.media.photos, 9) }).map((_, i) => (
                <MediaCell key={`p${i}`} type="PHOTO" />
              ))}
              {Array.from({ length: Math.min(loc.media.videos, 3) }).map((_, i) => (
                <MediaCell key={`v${i}`} type="VIDEO" />
              ))}
              {Array.from({ length: Math.min(loc.media.keepsakes, 3) }).map((_, i) => (
                <MediaCell key={`k${i}`} type="KEEP" />
              ))}
            </div>
          )}
